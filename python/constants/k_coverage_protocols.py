...
def get_tessellation_class_for_tile(k_coverage_approach: tuple):
    ...
    if tile_name == "square":
        ...
        if stochastic_sensing:
            return IrregularHexagon1(sensing_radius=int(min_stochastic_radius),
                                     area_length=int(cf.AREA_LENGTH))
        else:
            return IrregularHexagon1(sensing_radius=int(cf.COVERAGE_RADIUS),
                                     area_length=int(cf.AREA_LENGTH))

    elif tile_name == "slhex":
        if stochastic_sensing:
            return SlicedHexagon(sensing_radius=int(min_stochastic_radius),
                                 area_length=int(cf.AREA_LENGTH))
        else:
            return SlicedHexagon(sensing_radius=int(cf.COVERAGE_RADIUS),
                                 area_length=int(cf.AREA_LENGTH))
